    ...
    <v-flex xs12 md10 md8>
      <v-card>
        <v-toolbar dark color="primary">
          <v-toolbar-title>{{title}} {{dataEdit.key}}</v-toolbar-title>
        </v-toolbar>
        <v-card-text>
...
export default {
  ...
  methods: {
    ...
    async deleteTerm() {
      var form = { type: "studio", term_id: this.$route.params.id };
      ...
      if (response.success) {
        setTimeout(() => {
          this.$router.push({ path: "/admin/studio/edit" });
        }, 1000);
      }
    }
  },
